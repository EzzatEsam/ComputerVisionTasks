...
def download_file(url, dest_path):
    ...
    with open(dest_path, 'wb') as file, tqdm(
        desc=dest_path.name, total=total_size, unit='iB', unit_scale=True, unit_divisor=1024,
    ) as bar:
...
def extract_tar(tar_path, dest_folder):
    ...
    if final_extract_path.exists():
        print(f"Dataset already extracted to {final_extract_path}. Skipping extraction.")
...
def mat_date_to_python(matlab_datenum):
...
if __name__ == "__main__":
    ...
    # 3. Process Metadata
    mat_file = raw_data_path / "imdb.mat" # It is usually named imdb.mat inside imdb_crop
    ...
    if not mat_file.exists():
        ...
        if (raw_data_path / "wiki.mat").exists():
             ...
             raise FileNotFoundError(f"Could not find .mat metadata file in {raw_data_path}")
    ...
    # 8. Prepare Jobs
    train_jobs = [(found_files[row.filename], TRAIN_IMGS_FOLDER) for row in train_df.itertuples()]
    val_jobs = [(found_files[row.filename], VAL_IMGS_FOLDER) for row in val_df.itertuples()]
    ...
    train_df = train_df[train_df['filename'].isin([f.name for f in files_in_train])].reset_index(drop=True)
    val_df = val_df[val_df['filename'].isin([f.name for f in files_in_val])].reset_index(drop=True)
    # 11. Save CSVs
