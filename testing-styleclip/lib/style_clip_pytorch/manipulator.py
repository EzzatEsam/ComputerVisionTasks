...
class Manipulator:
    ...
    *** Attributes ***
    ...
    def __init__(
        ...
        dataset_name="",
        detector_model_path=Path("pretrained/shape_predictor_68_face_landmarks.dat"),
        tensors_parent_path: Path = Path("tensor/"),
    ):
        """Initialize
        ...
        self.W = torch.load(tensors_parent_path / f"W{dataset_name}.pt")
        self.S = torch.load(tensors_parent_path / f"S{dataset_name}.pt")
        self.S_mean = torch.load(tensors_parent_path / f"S_mean{dataset_name}.pt")
        self.S_std = torch.load(tensors_parent_path / f"S_std{dataset_name}.pt")

        self.S = {layer: self.S[layer].to(device) for layer in G.style_layers}
        self.styles = {
            layer: self.S[layer][start_ind : start_ind + num_images]
        ...
        if face_preprocess:
            self.landmarks_detector = FaceLandmarksDetector(detector_model_path)
        ...
        self.manipulate_layers = [
            layer for layer in G.style_layers if "torgb" not in layer
        ]
        ...
        if inv_mode == "w":
            ...
            for imgpath in imgpaths:
                ...
                target_pil = target_pil.resize(
                    (self.G.G.img_resolution, self.G.G.img_resolution),
                    PIL.Image.LANCZOS,
                )
                target_uint8 = np.array(target_pil, dtype=np.uint8)
                targets.append(
                    torch.Tensor(target_uint8.transpose([2, 0, 1])).to(self.device)
                )
    ...
    def synthesis_from_styles(self, styles, start_ind, end_ind):
        ...
        imgs = [
            self.G.synthesis_from_stylespace(
                self.latent[start_ind:end_ind], style_
            ).cpu()
...
def extract_global_direction(G, device, lst_alpha, num_images, dataset_name=""):
    ...
    latent = torch.randn(
        [1, G.to_w_idx[f"G.synthesis.b{resolution}.torgb.affine"] + 1, 512]
    ).to(
        device
    )  # 1024 -> 18, 512 -> 16, 256 -> 14
    ...
    for layer in manipulate_layers:
        ...
        for channel_ind in tqdm(range(channel_num), total=channel_num):
            ...
            for img_ind in range(0, nbatch):  # batch size 10 * 2
                ...
                synth_imgs = [
                    (synth_img.permute(0, 2, 3, 1) * 127.5 + 128)
                    .clamp(0, 255)
                    .to(torch.uint8)
                    .numpy()
...
if __name__ == "__main__":
    ...
    if runtype == "test":  # test manipulator
        ...
        num_images = 100
        lst_alpha = [-5, 5]
        extract_global_direction(
            G, device, lst_alpha, num_images, dataset_name=dataset_name
        )
