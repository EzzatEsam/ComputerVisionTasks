...
class FaceLandmarksDetector:
    ...
    def __init__(
            self, 
            model_path=Path('pretrained/shape_predictor_68_face_landmarks.dat'), 
        ):

        self.detector = LandmarksDetector(str(model_path))
        self.timestamp = int(time.time())
        sys_temp_dir = tempfile.gettempdir()
        self.tmp_src = f'{sys_temp_dir}/{self.timestamp}_src.png'
        self.tmp_align = f'{sys_temp_dir}/{self.timestamp}_align.png'
